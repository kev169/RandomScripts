...
class gui:
    ...
    def SetupTree(self):
        ...
        for item in self.data.split("\n"):
            if " fun_" in item and not item.strip().endswith(";"):
                functionName = "fun_"+item.split(" fun_")[1].split("(")[0]
                #print("Function %s"%(functionName))
                if "fun_" in functionName:
    ...
    def GetValue(self, var, x,y):
        ...
        if self.label1data != 'Full Code':
            ...
            self.functiontext = "%s"%(self.label1data) + self.data.split("%s"%(self.label1data))[1].split("\n}")[0] + "\n}"
            mytextview = self.builder.get_object("textview1").get_buffer()
    ...
    def RenameFunc(self, var):
        ...
        entryvalue = "fun_"+entry1.get_text()
        #print(entryvalue)
        self.data = self.data.replace(self.label1data, entryvalue)
        self.SetupTree() 
        self.functiontext = self.data.split("%s"%(entryvalue))[1].split("\n}")[0]
        mytextview = self.builder.get_object("textview1").get_buffer()
        mytextview.set_text(self.data)
        #mytextview = self.builder.get_object("textview1").get_buffer()
